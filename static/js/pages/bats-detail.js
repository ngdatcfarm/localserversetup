 ...
 * Per-barn detail view with 2x2 control grid, settings, and history.
 * Linked from the bats dashboard (/bats).
 */
const { ref, reactive, onMounted, computed, onUnmounted, watch } = Vue;
const { useRoute, useRouter } = VueRouter;

export default {
    props: {
        barnId: { type: [String, Number], default: '' },
    },
    setup(props) {
        ...
        async function onBarnChange() {
            ...
            if (selectedBarnId.value && route.params.barnId != selectedBarnId.value) {
                router.replace('/bats/' + selectedBarnId.value);
            }
        }
        ...
        const stopWatchMoving = watch(anyMoving, (moving) => {
            if (moving) startAutoRefresh();
            else stopAutoRefresh();
        }, { immediate: true });

        // Watch route param changes (user clicks a different barn elsewhere)
        const stopWatchRoute = watch(() => props.barnId, async (newId) => {
            if (newId && newId != selectedBarnId.value) {
                selectedBarnId.value = newId;
                await onBarnChange();
            }
        });
        ...
        onMounted(async () => {
            await loadBarns();
            // Priority: route prop > first barn
            if (props.barnId) {
                selectedBarnId.value = props.barnId;
                await onBarnChange();
            } else if (barns.value.length === 1) {
    ...
    <div class="bats-control-page">
        <div class="page-header">
            <div class="flex items-center gap-3">
                <button @click="backToDashboard" class="cf-btn-ghost-sm" title="Quay lại">←</button>
