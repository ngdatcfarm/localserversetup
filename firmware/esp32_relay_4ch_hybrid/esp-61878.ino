...
void setup() {
    ...
    // Setup cloud MQTT with its own WiFiClient (connect but don't send heartbeat)
...
void loop() {
    ...
    // IMPORTANT: Check BOTH connected() AND state()==3, because connected() can be true even when state=0
    bool localMqttHealthy = localMqttClient.connected() && localMqttClient.state() == 3;
    if (!localMqttHealthy) {
        if (now - lastLocalMqttReconnectAttempt >= MQTT_RECONNECT_DELAY) {
            lastLocalMqttReconnectAttempt = now;
            // Force disconnect first if TCP is connected but state is broken
            if (localMqttClient.connected()) {
                Serial.println("Local MQTT state stuck - forcing disconnect...");
...
void connectLocalMqtt() {
    ...
    // Connect WITHOUT credentials (broker allows anonymous)
    bool connected;
    if (strlen(LOCAL_MQTT_USER) > 0) {
        connected = localMqttClient.connect(DEVICE_CODE, LOCAL_MQTT_USER, LOCAL_MQTT_PASS);
    } else {
        connected = localMqttClient.connect(DEVICE_CODE);
    }

    if (connected) {
        Serial.println("OK");
        Serial.print("  immediate state: ");
        Serial.println(localMqttClient.state());

        // Wait for CONNACK with MORE loops and longer delays
        // CONNACK should arrive within ~500ms after TCP connect
        bool connackReceived = false;
        for (int i = 0; i < 100; i++) {
            // Call loop multiple times per iteration to process incoming data
            localMqttClient.loop();
            delay(5);
            localMqttClient.loop();
            delay(5);
        ...
        Serial.print("  state after wait: ");
        Serial.println(localMqttClient.state());

        // Now subscribe
        if (localMqttClient.state() == 3) {
            localMqttClient.subscribe(LOCAL_CMD_TOPIC);
            Serial.println("Subscribed to: " + String(LOCAL_CMD_TOPIC));
        } else {
            Serial.println("WARNING: Not fully connected, skipping subscribe");
        }
    } else {
...
void sendHeartbeat() {
    ...
    // Simple heartbeat - just publish, let loop() handle reconnection
    bool localSent = false;
    if (localMqttClient.connected()) {
        // Check state() - state=3 means MQTT_CONNECTED after CONNACK received
        int mqttState = localMqttClient.state();
        ...
        if (mqttState == 3) {
            ...
            Serial.println(localSent ? "OK" : "FAIL");
        } else {
            Serial.println("  state != 3, skipping publish");
        }

        if (!localSent) {
            // Disconnect and let loop() handle reconnect
            Serial.println("  forcing disconnect for reconnect...");
            localMqttClient.disconnect();
        }
        localMqttClient.loop();
    } else {
        Serial.println("Local MQTT disconnected - will reconnect in loop()");
    }

    // Cloud heartbeat - DISABLED to test if cloud interferes with local
    // Re-enable after local is stable
    /*
    bool cloudSent = false;
    if (cloudMqttClient.connected()) {
        cloudSent = cloudMqttClient.publish(CLOUD_HEARTBEAT_TOPIC, buffer, 1);
